      ...
      try {
        const [v, c, w, e] = await Promise.all([
          supabaseSelect<VisitRow>(
            "frontpage_visits",
            `select=created_at,session_id&order=created_at.desc&limit=${FETCH_LIMIT}`,
          ),
          ...
          supabaseSelect<EventRow>(
            "vault_events_prod",
            `select=block_timestamp,event_type,wallet_address,vault_address,tx_hash&event_type=in.(deposit,withdraw)&order=block_timestamp.desc&limit=${FETCH_LIMIT}`,
          ),
        ]);
        if (cancelled) return;
        setVisits(v);
