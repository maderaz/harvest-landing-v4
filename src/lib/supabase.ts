...
// Checked write: like supabaseInsert but returns the outcome so callers
// that need confirmation (e.g. the admin Hide panel) can surface a real
// error instead of optimistically assuming success. Returns the HTTP
// status and the response body text on failure so "table missing" /
// "RLS denied" are visible rather than swallowed.
...
export async function supabaseInsertChecked(
  table: string,
  row: Record<string, unknown>,
): Promise<WriteResult> {
  if (!configured()) {
    return { ok: false, status: 0, error: "Supabase not configured (missing NEXT_PUBLIC_SUPABASE_URL / ANON_KEY)." };
  }
  try {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, {
      ...
      headers: {
        ...
        Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
        "Content-Type": "application/json",
        // upsert so re-hiding an already-present slug is not a 409
...
// Checked delete: returns status + body on failure (see supabaseInsertChecked).
export async function supabaseDeleteChecked(
  table: string,
  filter: string,
): Promise<WriteResult> {
  if (!configured()) {
