...
data.landscape = freezeStampIfUnchanged(data.landscape, {
  ...
  note:
    "Daily-indexed XRP-denominated DeFi TVL across venues with a continuous daily series, plus Upshift's Flare vault TVL (protocol-level, its entire Flare presence is XRP). Venues newly switched to onchain sourcing rebuild their daily series forward from the switch, so they join this line as history accumulates; their current TVL is always in the day's total. Sources: onchain reads (Base and Flare) and the Spectra API.",
