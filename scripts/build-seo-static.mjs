...
- [XRP Rich List](${SITE_URL}/xrp-rich-list): the balance distribution of every funded XRP Ledger account, read by walking the ledger's own state tree rather than from an explorer's rich-list endpoint. Covers the largest accounts with named holders where a source can be shown, the minimum balance for each percentile tier (top 0.1%, 1%, 5%, 10%, 25%, 50%), accounts and XRP held per decade band, and a client-side percentile calculator that needs no address. Machine-readable data (snapshot JSON + three CSVs) at ${SITE_URL}/data/xrp-rich-list/index.json. Refreshed from the ledger; the page's date is the pinned ledger's own close time.
- [XRP Yield Ranking](${SITE_URL}/report/xrp-yield-ranking): every XRP-denominated DeFi yield source (XRP, FXRP, stXRP, cbXRP, wXRP) ranked by real 30-day rate. Covers lending, vaults, liquid staking, fixed-rate Spectra Principal Tokens and liquidity pools, split by single- vs dual-exposure, across Flare and Base. XRP has no native staking, so these are the real onchain rates, measured onchain (Base and Flare) with the Spectra API, refreshed hourly. Machine-readable data (per-product JSON + daily-rate CSV) at ${SITE_URL}/data/xrp-yield/index.json. All venues are third-party protocols, not Harvest products.
- [Best Crypto Casino Bonus](${SITE_URL}/best-crypto-casino-bonus): crypto casino welcome bonuses ranked by the size of the advertised offer, with the playthrough each one attaches. Records the wagering multiple and what it applies to, the qualifying deposit, published withdrawal wording, payout coins and lobby categories, each against the operator page it was read from and the date it was read; figures without a source are marked unconfirmed rather than printed as fact. Includes a calculator that prices a deposit against one offer, and the turnover every listed bonus asks for at its full advertised cap. Outbound links are commercial and labelled as such.
- [Aerodrome LP Yield Ranking](${SITE_URL}/report/aerodrome): the Aerodrome (Base) liquidity pools Harvest auto-compounds, ranked by real onchain pool APR (AERO gauge emissions + swap fees, measured onchain), with Harvest's auto-compounded 30-day APY beside each pool. Scoped to the pools Harvest operates, not a full Aerodrome index. Machine-readable data (snapshot JSON + flat CSV) at ${SITE_URL}/data/aerodrome/index.json.
- [Stablecoin Yield Ranking](${SITE_URL}/report/stablecoin-yield-ranking): the top stablecoin rates (USDC, USDT/USDT0, DAI, USDS and more) compared side by side across Ethereum, Base, Arbitrum and Polygon: the five best venues per network across Aave v3, Compound v3, Morpho, Fluid, Spark, Sky and Euler, discovered via the Portals API, plus Harvest's own stablecoin strategies in the same rate-sorted table, badged by operator. Includes a savings-account comparison and a CC-BY dataset at ${SITE_URL}/data/stablecoin-yield/index.json. Refreshed hourly; the report's date reflects the newest observation, not the build time.
